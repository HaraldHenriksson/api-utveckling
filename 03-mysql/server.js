...
app.get("/movies/:movieId", async (req, res) => {
	...
	const db = await connection;
	const [rows] = await db.query(`SELECT * FROM imdb_movies WHERE id=?`, [
		movieId,
	]);
...
app.patch("/movies/:movieId", async (req, res) => {
	...
	try {
		const result = db.query("UPDATE movies SET ? WHERE id = ?", [
			req.body,
			req.params.movieId,
		]);
	} catch (err) {
...
app.delete("/movies/:movieId", async (req, res) => {
	...
	try {
		const result = db.query("DELETE FROM movies WHERE id = ?", [
			req.params.movieId,
		]);
	} catch (err) {
		res.status(500).send({
			message: "You try to delete already deleted movie?!",
		});
	}
